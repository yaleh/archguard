...
**状态**: Draft (v3 — post-review revision)
**日期**: 2026-03-30
...
| v3 | Architecture review: fix incorrect codebase references (ml-matrix not installed, jl/ not implemented, co-change uses Jaccard not ratio); correct co-change matrix reconstruction requirement (top-10 edge list insufficient); tighten FIM analogy language; fix package-level aggregation acceptance criterion; add missing dependency/risk items |
...
此前的分析使用 co-change 矩阵作为 FIM 代理。**注意**：实际代码（`src/cli/git-history/history-aggregator.ts:buildCochangeIndex`）使用 Jaccard 相似度 $C_{ij} = \text{joint}(i,j) / (\text{commits}(i) + \text{commits}(j) - \text{joint}(i,j))$，而非简单比值。这一代理存在三个结构性缺陷：

| 缺陷 | 说明 |
|------|------|
| **非正半定** | Jaccard 相似度矩阵虽然对称，但不保证正半定（Jaccard 不是内积诱导的距离），特征值可为负 |
...
**数学诚实性说明**：严格意义上，$C^\top C$ 是覆盖矩阵的 Gram 矩阵，而非严格的 Fisher 信息矩阵。真正的 FIM 需要 $I_{ij} = -E[\partial^2 \ln p(x|\theta) / \partial\theta_i \partial\theta_j]$，这要求一个完整的参数化概率模型 $p(x|\theta)$ 和对 $\theta$ 的微分。$C^\top C$ 是一个**合理的一阶近似**——当每个文件独立地以概率 $\pi_f$ 被"测试触及"时，$C^\top C / T$ 趋近于 Bernoulli 模型的 FIM。本提案在后文中使用 "FIM" 指代这一 Gram 矩阵近似，读者应注意这一区别。
...
| Co-change 分析 | `src/cli/git-history/history-aggregator.ts` | 间接——`buildCochangeIndex()` 提供 Jaccard 协变强度。**但仅存储每文件 top-10 邻居，非完整矩阵**——Mantel test 需要从原始 commit 数据重建全矩阵 |

### 缺口

1. **istanbul/c8 JSON 报告解析器**——不存在。需要把 `coverage-final.json` 解析为 $C_{tf}$ 矩阵。
2. **Gram 矩阵计算 + 特征值分解**——不存在。`proposal-jl-intrinsic-dimension` 设计了使用 `ml-matrix` 的 SVD，**但该 proposal 尚未实现——`src/analysis/jl/` 目录不存在，`ml-matrix` 未在 `package.json` 中安装**。本提案需要自行引入 SVD 依赖。
3. **Mantel test**——不存在。需要矩阵相关性检验（置换检验）。
4. **纵向快照存储**——`proposal-jl-intrinsic-dimension` 设计了 `ArchHealthHistory` schema，**但尚未实现**。本提案需自行设计快照存储。
5. **完整 co-change 矩阵**——不存在。`buildCochangeIndex()` 仅输出每文件 top-10 邻居（`CochangeEdge[]`），Mantel test 需要全 $F \times F$ 矩阵。新增独立的 `buildFullCochangeMatrix()` 函数（在 `src/analysis/fim/cochange-matrix-builder.ts` 中），从原始 `CommitRecord[]` 重建完整 Jaccard 矩阵，不修改现有 `buildCochangeIndex`。

### 可复用的现有组件

| 组件 | 来源 | 复用方式 | 就绪状态 |
|------|------|---------|---------|
| SVD 库 | 需新增 `ml-matrix` 或等效库 | 特征值分解 | **未安装** — 需 `npm install ml-matrix` |
| `ArchHealthHistory` | proposal-jl-intrinsic-dimension (设计) | 扩展快照 schema | **未实现** — 本提案需自建 |
| Co-change 原始数据 | `CommitRecord[]` from `git-log-reader.ts` | 新建 `cochange-matrix-builder.ts` 从原始 commits 构建完整 Jaccard 矩阵 | `CommitRecord` 已有 — 需新建矩阵构建器 |
| `isTestFile()` / `extractTestStructure()` | 各语言插件 | 测试文件发现 | 已实现 |
| `TestCoverageMapper` | `src/analysis/test-coverage-mapper.ts` | Import 依赖分析逻辑参考 | 已实现 |
| 实体/关系图 | ArchJSON (`src/types/index.ts`) | 文件→实体映射 | 已实现 |
...
Mantel test 检验两个距离/相异矩阵之间的相关性：

1. **构造 co-change 完整矩阵**：现有 `buildCochangeIndex()` 仅保留 top-10 邻居。在 `src/analysis/fim/cochange-matrix-builder.ts` 中新建 `buildFullCochangeMatrix(commits, fileIds): number[][]`，从原始 `CommitRecord[]` 构建与覆盖 FIM 同维度的完整 Jaccard 相似度矩阵（对称，对角线为 1）。不修改现有 `buildCochangeIndex`。
2. 将覆盖 FIM $I_{\text{cov}}$ 和 co-change 矩阵 $I_{\text{cc}}$ 分别转换为距离矩阵。**注意**：归一化方式需要考虑两矩阵的值域不同—— $I_{\text{cov}}$ 的元素是整数（共同覆盖测试数），$I_{\text{cc}}$ 的元素是 Jaccard 相似度 $\in [0,1]$。推荐先做行列标准化或排名变换（Spearman Mantel test）而非简单的 $D_{ij} = 1 - I_{ij}/\max(I)$，以避免量纲差异影响。
3. 计算 Pearson（或 Spearman）相关 $r_{\text{obs}}$ 在**严格上三角**元素之间（排除对角线——对角线是自信息，不提供配对距离信号）
4. 对 $I_{\text{cc}}$ 做 $N = 999$ 次行列同步置换，每次重算 $r$
5. $p = (\text{count}(r_{\text{perm}} \geq r_{\text{obs}}) + 1) / (N + 1)$
6. $p < 0.05$ → co-change 是覆盖 FIM 的有效代理

### 数据流

```
Phase 1: import graph          Phase 2: vitest --coverage → coverage-final.json
  (TestCoverageMapper)                          ↓
         ↓                              coverage-parser.ts
  coverage-parser.ts                    (解析 per-test coverage → C[test][file])
  (import deps → C[test][file])
         ↓                                      ↓
         └──────────────┬───────────────────────┘
                        ↓
                 fim-builder.ts
                (I = CᵀC, SVD via ml-matrix, κ, N_eff)
                        ↓
            ┌───────────┴───────────┐
            ↓                       ↓
     mantel-test.ts          fim-snapshot.ts
  (I_cov vs I_cochange       (追加到
   需要完整 co-change         fim-history.json)
   矩阵 — 从 git commits
   重建，非 top-10 edge)
            ↓                       ↓
 ...
 * Phase 2 将新增 buildCoverageMatrixFromCoverage() 使用运行时数据
 */
...
包级覆盖矩阵 C_pkg = clamp(C × G, 0, 1)    (T × P, 二值化)
包级 FIM I_pkg = C_pkg^T C_pkg               (P × P)
```

**注意**：$C \times G$ 的元素可能 $> 1$（测试覆盖包内多个文件时），需要 clamp 到 $\{0,1\}$ 以保持二值矩阵语义。否则 $I_{\text{pkg}}$ 的含义从"共同覆盖的测试数"变为"加权共同覆盖"，失去 FIM 的概率解释。
...
export interface MantelTestOptions {
  ...
  seed?: number;          // 可选 seed，用于测试确定性
}

/**
 * 检验覆盖 FIM 与 co-change 矩阵之间的矩阵相关性。
 * 输入必须为相同维度（包级聚合后）。
 * 使用 Spearman rank correlation 处理值域差异。
 */
...
`proposal-jl-intrinsic-dimension` 的 `ArchHealthHistory` schema 尚未实现（`src/analysis/jl/` 不存在）。本提案在 `src/analysis/fim/types.ts` 中自建快照类型，后续 JL 实现时可合并：

```typescript
// src/analysis/fim/types.ts
...
src/analysis/fim/
  ...
  fim-builder.ts              # I = CᵀC, SVD (via ml-matrix), κ, N_eff, 脆弱点
  mantel-test.ts              # 矩阵相关性置换检验
  cochange-matrix-builder.ts  # 从 CommitRecord[] 构建完整 F×F Jaccard 矩阵
...
tests/unit/analysis/fim/
  ...
  fim-builder.test.ts
  mantel-test.test.ts
  cochange-matrix-builder.test.ts
  fim-snapshot.test.ts
  fim-integration.test.ts          # CLI pipeline integration tests

tests/unit/cli/mcp/tools/
  fim-tools.test.ts                # MCP tool tests
```
...
| `src/analysis/fim/cochange-matrix-builder.ts` | 新建 | 从 CommitRecord[] 构建完整 Jaccard 矩阵（不截断 top-N） |
...
| `src/cli/mcp/mcp-server.ts` | 修改 | 注册新 MCP 工具（注意实际文件名是 `mcp-server.ts` 而非 `server.ts`） |
| `package.json` | 修改 | 新增 `ml-matrix` 依赖（或等效 SVD 库） |
...
| 风险 | 影响 | 缓解 |
|------|------|------|
| `ml-matrix` 未安装 | Phase 1 无法进行 SVD | 本提案需作为前置步骤安装 `ml-matrix`（~50KB，无 native deps） |
| JL proposal 未实现 | 无 `ArchHealthHistory`、无 `src/analysis/jl/` | 本提案自建 `FIMSnapshot` 和存储逻辑，后续 JL 实现时可合并 |
| Co-change 仅存 top-10 | Mantel test 需要完整矩阵 | 新建 `src/analysis/fim/cochange-matrix-builder.ts`，从原始 `CommitRecord[]` 独立构建完整 Jaccard 矩阵，不修改现有 `buildCochangeIndex` |
| Import 近似文件级不可信 | Phase 1 scope 受限于包级 | 已在 Phase 2 升级路径中明确；Phase 1 文档和 CLI 输出需清晰标注近似来源 |
| 小项目包数 < 10 | Mantel test 统计功效不足 | 文件级 Mantel test 作为补充（但需 Phase 2a 数据） |
| `buildFullCochangeMatrix` 内存 | $F^2$ 对于大项目（$F > 1000$）可能占用大量内存 | 包级聚合后矩阵通常 $P < 50$，风险可控；文件级可用稀疏表示 |
...
- 不修改 ArchJSON schema
- 需新增 `ml-matrix` 依赖（**注意**：此前 JL proposal 设计了引入该依赖但尚未实现——`ml-matrix` 当前不在 `package.json` 中）
...
5. 人造 $C$（所有测试覆盖所有文件，即全 1 矩阵）→ $I$ 为秩 1（$\lambda_1 = T \cdot F$, 其余 $\lambda = 0$），$\kappa = \infty$，$N_{\text{eff}} = 1$
...
13. 包级 FIM 的对角线 $I_{\text{pkg}}[p][p]$ = 覆盖包 $p$ 中**至少一个文件**的测试数（即 $\text{clamp}(C \times G)$ 第 $p$ 列的 $L_1$ 范数）。**注意**：这**不等于**该包内所有文件 $I_{ii}$ 之和——同一测试覆盖包内多个文件时只计一次
...
| $C^\top C$ 作为 FIM 的数学严格性 | Gram 矩阵是 FIM 的一阶近似，而非严格推导。文献中 Fisher 信息要求 $-E[\partial^2 \ln p / \partial\theta_i \partial\theta_j]$，二值覆盖矩阵的 Gram 矩阵缺乏完整的参数化概率模型支撑。在对外沟通时应使用 "coverage Gram matrix as FIM proxy" 而非 "true FIM" | 在文档和 CLI 输出中明确标注为近似 |
| Mantel test 中两矩阵值域不对齐 | 覆盖 FIM 元素为整数（0..T），co-change Jaccard 为 [0,1]。简单归一化可能不够 | 推荐使用 Spearman rank correlation 或 row-standardization |
