...
L0 Worker（`.claude/loop.md`）需要周期性触发才能消费 `agent-run` 队列。四种方案各有权衡。
...
## 方案 A2：`/loop` CronCreate（会话内定时）

在 Claude Code session 中启动定时 loop：

```
/loop 5m @.claude/loop.md
```

**机制**：Claude Code 内部调用 `CronCreate`，在进程内存中注册一个每 5 分钟触发的任务。与系统 `crontab` 完全无关。
...
**限制**：
- **Session 关闭即停止**（存储在内存中，非持久化）
- 调度粒度最小 5 分钟
- Session 长时间运行会积累 context，影响性能

**管理命令**（在 Claude Code 中）：
```
CronList              # 查看活跃定时任务（工具调用）
CronDelete <id>       # 删除指定任务
```

**适用场景**：当前 session 需要持续执行、不想手动触发但也不需要跨 session 持久化
...
| 维度 | A 手动 | A2 CronCreate | B /schedule | C 系统 cron |
|------|--------|--------------|------------|------------|
| 无人值守 | ❌ | ✅（session 内） | ✅ | ✅ |
| Session 无关 | ❌ | ❌ | ✅ | ✅ |
| 零配置 | ✅ | ✅ | ✅ | ❌ |
| 本地日志 | ❌ | ❌ | ❌ | ✅ |
| 持久化 | — | 7 天/session | 云端持久 | 机器存活期 |
| token 成本 | 按需 | 固定轮询 | 固定轮询 | 固定轮询 |
| 当前使用 | ✅ | — | — | — |
