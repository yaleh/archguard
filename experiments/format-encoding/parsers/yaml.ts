...
export function parse(text: string): C {
  let doc: RawDoc;
  try {
    doc = parseYaml(text) as RawDoc;
  } catch (e) {
    throw new Error(`yaml: failed to parse YAML: ${(e as Error).message}`);
  }

  if (!doc || !Array.isArray(doc.entities)) {
    throw new Error('yaml: missing or invalid "entities" sequence at top level');
  }
  ...
  for (const raw of doc.entities) {
    if (!raw.id || !raw.name || !raw.type) {
      throw new Error(`yaml: entity missing required field(s) id/name/type: ${JSON.stringify(raw)}`);
    }
