...
// Extra params from --param key=value args (e.g. --param enable_thinking=false)
...
for (let i = 3; i < process.argv.length; i++) {
  if (process.argv[i] === '--param' && process.argv[i+1]) {
    ...
    try { EXTRA_PARAMS[k] = JSON.parse(v); }
    catch { EXTRA_PARAMS[k] = v === 'false' ? false : v === 'true' ? true : isNaN(Number(v)) ? v : Number(v); }
  } else if (process.argv[i] === '--timeout' && process.argv[i+1]) {
    TIMEOUT_MS = Number(process.argv[++i]);
  }
}
...
for (const task of sample) {
  for (const level of LEVELS) {
    ...
    for (let k = 1; k <= K; k++) {
      const t0 = Date.now();
      try {
        const res = await client.chat({ model: MODEL, messages: [{ role: 'user', content: prompt }], params: { temperature: 0.2, max_tokens: 1024, ...EXTRA_PARAMS }, timeoutMs: TIMEOUT_MS });
