...
export const mcpTools: McpToolMetadata[] = [
  ...
  mcpTool({
    toolName: 'archguard_get_ccb',
    title: 'Cognitive Context Bundle',
    summary: 'Return a cached Cognitive Context Bundle for a source file.',
    category: 'query',
    callFirst: ['archguard_analyze', 'archguard_analyze_git'],
    limitations: [
      'This is an agent-oriented context bundle without a dedicated terminal CLI equivalent.',
      'LLM-layer fields such as documentation.deFactoSpec and documentation.freshnessWarning are intentionally null in stored bundles.',
    ],
    parameters: ['projectRoot', 'archDir', 'filePath', 'forceRefresh'],
    requiredParameters: ['filePath'],
    parameterDescriptions: {
      archDir: 'Path to .archguard directory (default: <projectRoot>/.archguard).',
      filePath: 'Path to the source file, relative to projectRoot or absolute.',
      forceRefresh: 'When true, bypass the CCB cache and reassemble from source artifacts.',
    },
    verification: 'npm test -- tests/unit/cli/mcp/ccb-tool.test.ts',
  }),
  mcpTool({
    toolName: 'archguard_get_cognitive_summary',
    title: 'Cognitive Summary',
    summary: 'Return compact structural digests for requested entity names.',
    category: 'query',
    callFirst: ['archguard_analyze'],
    limitations: [
      'This is an agent-oriented batch digest without a dedicated terminal CLI equivalent.',
      'Test coverage and git risk fields are null when the corresponding artifacts are absent.',
    ],
    parameters: ['projectRoot', 'archDir', 'entities'],
    requiredParameters: ['entities'],
    parameterDescriptions: {
      archDir: 'Path to .archguard directory (default: <projectRoot>/.archguard).',
      entities: 'Entity names to summarize (1-20 names, exact match).',
    },
    verification: 'npm test -- tests/unit/cli/mcp/cognitive-summary-tool.test.ts',
  }),
  mcpTool({
  ...
  mcpTool({
    toolName: 'archguard_get_package_metrics',
    title: 'Package Metrics',
    summary: 'Aggregate fan-in, fan-out, and cycle count per package.',
    category: 'query',
    callFirst: queryArtifactCallFirst,
    parameters: ['projectRoot', 'scope', 'packageName'],
    parameterDescriptions: {
      packageName:
        'Filter results to a single package name. Omit to return metrics for all packages.',
    },
    verification: 'npm test -- tests/unit/cli/mcp/tools/package-metrics-tool.test.ts',
  }),
  mcpTool({
