...
export class IsomorphicMermaidRenderer {
   ...
   *
   * Caller must ensure SVG has been processed by postProcessSVG() before
   * passing here. This method uses the svg string as-is without any further
   * post-processing.
   */
   ...
   * Legacy path: handles its own pre-processing (inlineEdgeStyles) before
   * delegating to convertSVGToPNG, which expects a pre-processed SVG.
   */
