...
export class TestCoverageRenderer {
  render(analysis: TestAnalysis, archJson: ArchJSON): string {
    ...
    const wellTested: { label: string; entityId: string }[] = [];
    const partiallyTested: { label: string; entityId: string }[] = [];
    const notTested: { label: string; entityId: string }[] = [];
    ...
    for (const entity of archJson.entities) {
      if (entity.type === 'class' || entity.type === 'interface') {
        ...
        const entry = { label, entityId: entity.id };
        if (score >= 0.7) wellTested.push(entry);
        else if (score >= 0.3) partiallyTested.push(entry);
        else notTested.push(entry);
      }
    }
    ...
    for (const entry of wtSlice) {
      lines.push(`    ${this.nodeId(entry.label, entry.entityId)}["${entry.label}"]`);
    }
    ...
    if (debugFiles.length > 0) {
      ...
      for (const label of doSlice) {
        lines.push(`    ${this.nodeId('do_' + label, label)}["${label}"]`);
      }
  ...
  private nodeId(label: string, entityId?: string): string {
    const base = label.replace(/[^a-zA-Z0-9_]/g, '_').replace(/^(\d)/, '_$1');
    if (entityId) {
      const idPart = entityId.replace(/[^a-zA-Z0-9_]/g, '_');
