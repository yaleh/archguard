...
describe('registerGIMTools', () => {
  beforeEach(() => {
    ...
    vi.mocked(loadEngine).mockResolvedValue({
      engine: { getCycles: vi.fn(() => []) } as unknown as Awaited<
        ReturnType<typeof loadEngine>
      >['engine'],
  ...
  it('returns highInfluencePackages sorted by fanIn DESC, top 5 only', async () => {
    ...
    if (data.highInfluencePackages.length >= 2) {
      expect(data.highInfluencePackages[0].fanIn).toBeGreaterThanOrEqual(
        data.highInfluencePackages[1].fanIn
      );
    }
  });
