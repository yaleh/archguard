/**
 * TASK-63 Phase C — Java pack parity integration test.
 *
 * Asserts that RuleBasedLanguagePlugin (driven by the declarative Java pack)
 * produces output within ±10% of the current imperative JavaPlugin on the
 * `tests/fixtures/java-simple/` fixture project, measured by entity count and
 * relation count.
 */
...
const FIXTURE = fileURLToPath(new URL('../../../fixtures/java-simple', import.meta.url));
...
describe('java pack parity vs imperative JavaPlugin', () => {
  ...
  it('produces entity/relation counts within ±10% of the imperative plugin', async () => {
    ...
    // Both plugins use the portable WASM backend: native tree-sitter is an
    // optional accelerator not guaranteed present in every environment.
    const imperative = new JavaPlugin(wasmParserBackend);
    ...
    process.stdout.write(
      `[java-parity] entities rule=${ruleResult.entities.length} ` +
